...
if [ -n "$HEALTH_CMD" ]; then
    # Use configured health check (add -f flag if curl command)
    HEALTH_CMD_FIXED="$HEALTH_CMD"
    if [[ "$HEALTH_CMD" == curl* ]] && [[ "$HEALTH_CMD" != *"-f"* ]]; then
        HEALTH_CMD_FIXED="${HEALTH_CMD/curl /curl -f }"
    fi

    if eval "$HEALTH_CMD_FIXED" > /dev/null 2>&1; then
        ...
        echo "  Health command: $HEALTH_CMD_FIXED"
        echo ""
    fi
